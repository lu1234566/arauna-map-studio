...
describe("conservative map passability", () => {
  ...
  it("distinguishes confirmed walkable, surf/conditional and unknown behaviors", () => {
    ...
    expect(classifyBehavior(0xe1)).toBe("unknown"); // interactive behavior not proven walkable
  });
  ...
  it("uses strict and lenient connectivity separately", () => {
    const map: MapData = {
      width: 3,
      height: 1,
      metatiles: Uint16Array.of(1, 2, 1),
      physical: Uint16Array.of(0x3000, 0x3000, 0x3000),
    };
    ...
    expect(grid.states).toEqual(["passable", "conditional", "passable"]);
    const strict = connectedComponents(grid, STRICT_PASSABLE);
    const lenient = connectedComponents(grid, LENIENT_PASSABLE);
    expect(strict[0]).not.toBe(strict[2]);
    expect(lenient[0]).toBe(lenient[2]);
  });
});
