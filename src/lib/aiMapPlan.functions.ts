...
async function callLovableGateway(data: PlannerRequest, apiKey: string) {
  ...
  const response = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
    ...
    body: JSON.stringify({
      model,
      temperature: 0.15,
      messages: [{ role: "user", content: plannerInstructions(data) }],
    }),
  });
...
export const planMapWithGemini = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
      ...
      try {
        ...
        if (!geminiKey) {
          return {
            ...
            message: `Lovable AI falhou: ${error instanceof Error ? error.message : String(error)}`,
          };
        }
      }
    }
    ...
    if (geminiKey) {
      try {
        ...
        return {
          ok: false as const,
          configured: true as const,
          message: `Gemini falhou: ${error instanceof Error ? error.message : String(error)}`,
        };
      }
    }
    ...
    return {
      ...
      message: "O AI Gateway ainda não foi provisionado neste projeto e não há GEMINI_API_KEY. O modo Interpretar local continua funcional para comandos precisos.",
    };
  });
