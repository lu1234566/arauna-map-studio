...
import { ArrowLeft, Download, FolderOpen, Image as ImageIcon, RefreshCw } from "lucide-react";
import {
...
export const Route = createFileRoute("/tilesets")({
  component: TilesetLab,
});
...
function TilesetLab() {
  ...
  const [message, setMessage] = useState("Carregue General (primary) e Petalburg (secondary). Tudo é processado localmente no navegador.");
  ...
  const build = async () => {
    ...
    try {
      ...
      setMessage(
        `Atlas montado: ${primaryMetatiles.count} primary + ${secondaryMetatiles.count} secondary metatiles.` +
  ...
  const exportPng = async () => {
    ...
    if (!blob) return;
    downloadBlob(blob, "general-petalburg-metatiles.png");
  };
    ...
    <div className="flex min-h-dvh flex-col bg-background text-foreground">
      <header className="flex h-12 shrink-0 items-center gap-3 border-b border-border bg-toolbar px-4">
        ...
        <div className="ml-auto flex gap-1">
          ...
          >
            <RefreshCw className={`size-3.5 ${busy ? "animate-spin" : ""}`} /> Montar atlas
          </button>
          <button
            type="button"
            disabled={!pair}
            onClick={() => void exportPng()}
            className="inline-flex h-8 items-center gap-1.5 rounded border border-border px-2 text-xs hover:bg-surface disabled:opacity-30"
          >
      ...
      <main className="grid min-h-0 flex-1 grid-cols-[320px_minmax(0,1fr)_260px] overflow-hidden">
        <aside className="overflow-y-auto border-r border-border bg-panel p-3">
          <p className="mb-3 text-xs leading-relaxed text-muted-foreground">
            Use os arquivos do repositório do jogo. Não há upload para servidor: o Chrome lê e renderiza tudo localmente.
          </p>
          <FileGroup
            title="Primary · General"
            root="data/tilesets/primary/general"
            value={primary}
            onChange={(patch) => update("primary", patch)}
            paletteHint="Selecione 00.pal até 05.pal"
          />
          <FileGroup
            title="Secondary · Petalburg"
            root="data/tilesets/secondary/petalburg"
            value={secondary}
            onChange={(patch) => update("secondary", patch)}
            paletteHint="Selecione 06.pal até 12.pal"
          />
          <div className="mt-3 rounded border border-border bg-canvas p-2 text-[10px] leading-relaxed text-muted-foreground">
            <b className="text-foreground">Por que dois tilesets?</b><br />
            Emerald carrega 512 tiles e 512 metatiles do primary. O secondary ocupa a faixa seguinte e pode reutilizar tiles/paletas do primary.
          </div>
        </aside>
          ...
          {!pair ? (
            <div className="grid h-full place-items-center text-center text-muted-foreground">
              <div>
                ...
                <p className="text-sm">O atlas real aparecerá aqui.</p>
                <p className="mt-1 text-xs">Carregue os arquivos à esquerda e clique em “Montar atlas”.</p>
              </div>
            </div>
          ) : (
            <div className="mx-auto w-fit">
              <div className="mb-2 flex items-center justify-between text-[10px] text-muted-foreground">
                <span>{records.length} metatiles renderizados</span>
                <span>16×16 px · pixel-perfect</span>
              </div>
        ...
        <aside className="overflow-y-auto border-l border-border bg-panel p-3">
          ...
          {warnings.length > 0 && (
            <div className="mt-3">
              <h3 className="panel-title mb-1">Avisos</h3>
              <ul className="space-y-1">
                {warnings.map((warning) => (
                  <li key={warning} className="rounded border border-warning/30 bg-warning/5 p-1.5 text-[10px] text-warning">
                    {warning}
                  </li>
                ))}
              </ul>
          ...
          <div className="mt-3 rounded border border-border p-2 text-[10px] leading-relaxed text-muted-foreground">
            O PNG exportado é um atlas achatado para visualização. O JSON guarda IDs, origem, behavior e layer type quando os arquivos de atributos foram fornecidos.
          </div>
        </aside>
      </main>
    </div>
...
async function readPalettes(files: File[]): Promise<Map<number, RgbColor[]>> {
  ...
  for (const file of files) {
    ...
    if (!match) continue;
    result.set(Number(match[1]), parseJascPalette(await file.text()));
  }
  return result;
}
...
function FileGroup({
  ...
  root: string;
  value: SideFiles;
  onChange: (patch: Partial<SideFiles>) => void;
  paletteHint: string;
}) {
  return (
    <section className="mb-3 rounded-md border border-border bg-background/20 p-2.5">
      <div className="mb-2">
        ...
        <p className="mt-0.5 break-all font-mono text-[9px] text-muted-foreground">{root}</p>
      </div>
      ...
      <label className="mt-2 block rounded border border-dashed border-border p-2 hover:bg-surface">
        <span className="flex cursor-pointer items-center gap-1.5 text-[11px]">
          <FolderOpen className="size-3.5" /> Paletas .pal
        </span>
        <span className="mt-0.5 block text-[9px] text-muted-foreground">
          {value.palettes.length ? `${value.palettes.length} arquivo(s)` : paletteHint}
        </span>
        <input
          className="hidden"
          type="file"
          accept=".pal"
          multiple
          onChange={(event) => onChange({ palettes: Array.from(event.target.files ?? []) })}
        />
      </label>
    </section>
  );
  ...
  return (
    <div className="mb-1 flex items-center gap-1">
      <button
        ...
        className="flex h-7 min-w-0 flex-1 items-center gap-1.5 rounded border border-border px-2 text-left text-[10px] hover:bg-surface"
      >
        <FolderOpen className="size-3 shrink-0" />
        <span className="min-w-0 truncate">{file?.name ?? label}</span>
      </button>
      {optional && <span className="text-[8px] text-muted-foreground">opc.</span>}
      <input
        ref={ref}
        className="hidden"
        type="file"
...
function MetatileButton({
  ...
  const drawRef = (canvas: HTMLCanvasElement | null) => {
  ...
  return (
    <button
      type="button"
      title={`Metatile ${id} · 0x${id.toString(16).toUpperCase().padStart(3, "0")}`}
      onClick={onSelect}
      className={`relative size-8 overflow-hidden bg-background p-0 ${selected ? "z-10 outline-2 outline-primary" : "hover:outline hover:outline-1 hover:outline-foreground/60"}`}
    >
      <canvas ref={drawRef} className="pixelated size-8" />
    </button>
  );
}
...
function SelectedInfo({ pair, id }: { pair: RenderTilesetPair; id: number }) {
  ...
  return (
    <section className="mt-3 border-t border-border pt-3">
      <h3 className="panel-title mb-1">Selecionado</h3>
      <dl className="space-y-1 font-mono text-[10px]">
        <InfoRow label="ID" value={`${id} / 0x${id.toString(16).toUpperCase().padStart(3, "0")}`} />
        <InfoRow label="Origem" value={source} />
        <InfoRow label="Local ID" value={String(localId)} />
        <InfoRow label="Behavior" value={attr ? `0x${attr.behavior.toString(16).toUpperCase().padStart(2, "0")}` : "—"} />
        <InfoRow label="Layer type" value={attr ? String(attr.layerType) : "—"} />
      </dl>
    </section>
  );
}
