...
function scopeFromAtlas(atlas: SavedRealAtlas | null) {
...
function ProceduralGenerator() {
  ...
  const toggleExit = (direction: keyof ProceduralExits) => {
    update("exits", { ...spec.exits, [direction]: !spec.exits[direction] });
  };

  const generate = () => {
    setResult(generateProceduralBlueprint(spec, patterns.patterns, smartPaths.presets, scopeFromAtlas(atlas)));
    setSaved("");
  };
  ...
  return (
    <div className="flex min-h-dvh flex-col bg-background text-foreground">
      <header className="flex h-12 shrink-0 items-center gap-3 border-b border-border bg-toolbar px-4">
        ...
        <div>
          <h1 className="text-sm font-semibold">Procedural Blueprint Generator</h1>
          <p className="text-[10px] text-muted-foreground">Seed + vocabulário verificado → Blueprint IA → Template GBA</p>
        </div>
        <div className="ml-auto flex items-center gap-2 text-[10px]">
          ...
          <span className={cn("rounded border px-2 py-1", atlas ? "border-success/30 bg-success/10 text-success" : "border-warning/30 bg-warning/10 text-warning")}>
      ...
      <main className="grid min-h-0 flex-1 grid-cols-[300px_minmax(420px,1fr)_330px] overflow-hidden">
        <aside className="overflow-y-auto border-r border-border bg-panel p-3">
          ...
          <label className="mb-2 block"><span className="field-label">Nome</span><input value={spec.name} onChange={(event) => update("name", event.target.value)} className="input-compact w-full" /></label>
          ...
          <div className="rounded border border-border bg-canvas p-2 text-[10px] leading-relaxed text-muted-foreground">
            O algoritmo só posiciona <b className="text-foreground">Patterns</b> cadastrados e só desenha conexões usando um <b className="text-foreground">Smart Path</b> existente. A saída intermediária é o mesmo contrato `arauna-map-blueprint-v1` usado pelo painel Blueprint IA.
          </div>
        </aside>
        ...
        <section className="min-w-0 overflow-y-auto bg-canvas p-4">
          <div className="mx-auto flex min-h-full max-w-[980px] flex-col gap-3">
            <div className="flex items-center justify-between">
              <div><h2 className="text-sm font-semibold">Prévia do layout</h2><p className="text-[10px] text-muted-foreground">A mesma seed produz o mesmo arranjo. Rotas usam busca em grade para contornar estruturas.</p></div>
            ...
            <div className="grid min-h-[480px] flex-1 place-items-center overflow-auto rounded-lg border border-border bg-background/40 p-4 shadow-inner">
              {!result ? <EmptyPreview title="Pronto para gerar" text="Escolha as peças e clique em Gerar." />
                : !result.ok ? <EmptyPreview title="Geração bloqueada" text={result.errors.join(" ")} error />
                  : previewMap ? <GbaMapPreview map={previewMap} atlas={atlas} result={result} />
            ...
            {result?.ok && (
              <div className="grid grid-cols-4 gap-2">
                ...
                <Stat label="Blueprint" value={String((result.blueprint?.patterns.length ?? 0) + (result.blueprint?.routes.length ?? 0))} />
                <Stat label="Template" value={String(result.compiled?.template?.elements.length ?? 0)} />
              </div>
            )}
        ...
        <aside className="overflow-y-auto border-l border-border bg-panel p-3">
          ...
          {!patterns.patterns.length ? (
            <div className="mb-3 rounded border border-warning/30 bg-warning/5 p-2 text-[10px] leading-relaxed text-warning">A Biblioteca está vazia. No Editor, copie estruturas aprovadas e salve-as em <b>Padrões</b>.</div>
          ) : (
            <>
              <PatternRoleList title="Marcos obrigatórios" hint="Cada item marcado é colocado uma vez e ligado ao hub." role="landmark" selected={spec.landmarkPatternIds} patterns={patterns.patterns} onToggle={togglePattern} />
              <PatternRoleList title="Preenchimento" hint="Sorteia entre os itens marcados até atingir Fillers." role="filler" selected={spec.fillerPatternIds} patterns={patterns.patterns} onToggle={togglePattern} />
            </>
          )}

          <section className="mt-4 border-t border-border pt-3">
            <h2 className="panel-title mb-2">Resultado</h2>
            {!result?.ok || !result.compiled?.template || !result.blueprint ? (
              <p className="text-[10px] leading-relaxed text-muted-foreground">Gere um layout válido para salvá-lo como Template ou reutilizar o Blueprint JSON.</p>
            ) : (
              <div className="space-y-2">
                ...
                <button type="button" onClick={() => copyText(`${JSON.stringify(result.blueprint, null, 2)}\n`)} className="inline-flex w-full items-center justify-center gap-1.5 rounded border border-border px-3 py-2 text-xs hover:bg-surface"><ClipboardCopy className="size-3.5" /> Copiar Blueprint JSON</button>
...
function PatternRoleList({ title, hint, role, selected, patterns, onToggle }: { title: string; hint: string; role: PatternRole; selected: string[]; patterns: MapPattern[]; onToggle: (role: PatternRole, id: string) => void }) {
  return (
    <section className="mb-4">
      <h3 className="text-[11px] font-semibold">{title}</h3><p className="mb-1.5 text-[9px] leading-relaxed text-muted-foreground">{hint}</p>
      <div className="max-h-52 space-y-1 overflow-y-auto pr-1">
        {patterns.map((pattern) => {
          const active = selected.includes(pattern.id);
          return <button key={pattern.id} type="button" onClick={() => onToggle(role, pattern.id)} className={cn("flex w-full items-center gap-2 rounded border p-1.5 text-left", active ? "border-primary/40 bg-primary/10" : "border-border bg-canvas hover:bg-surface")}><span className={cn("grid size-4 shrink-0 place-items-center rounded border text-[9px]", active ? "border-primary bg-primary text-primary-foreground" : "border-border")}>{active ? "✓" : ""}</span><span className="min-w-0 flex-1"><span className="block truncate text-[10px] font-medium">{pattern.name}</span><span className="block truncate text-[8px] text-muted-foreground">{pattern.category} · {pattern.width}×{pattern.height}</span></span></button>;
        })}
      </div>
    </section>
  );
...
function GbaMapPreview({ map, atlas, result }: { map: MapData; atlas: SavedRealAtlas | null; result: ProceduralBlueprintResult }) {
  ...
  return <div className="max-h-full max-w-full overflow-auto rounded border border-border bg-black/20 p-1 shadow-2xl"><canvas ref={canvasRef} className="block max-h-[68vh] max-w-full object-contain [image-rendering:pixelated]" /></div>;
}
  ...
  return (
    <div className="relative aspect-[5/4] w-full max-w-[720px] overflow-hidden rounded border border-border bg-panel shadow-xl">
      <svg className="absolute inset-0 size-full" viewBox={`0 0 ${width} ${height}`} preserveAspectRatio="none">
        {result.roads.map((road, index) => <polyline key={`${road.kind}-${road.label}-${index}`} points={road.points.map((point) => `${point.x + 0.5},${point.y + 0.5}`).join(" ")} fill="none" stroke="currentColor" strokeWidth="0.45" className="text-primary/70" vectorEffect="non-scaling-stroke" />)}
      </svg>
      {result.placements.map((placement, index) => <div key={`${placement.patternId}-${index}`} title={`${placement.role}: ${placement.patternId}`} className={cn("absolute grid place-items-center overflow-hidden rounded-sm border text-center text-[8px] font-semibold shadow", placement.role === "center" ? "border-primary bg-primary/25 text-primary" : placement.role === "landmark" ? "border-success/50 bg-success/15 text-success" : "border-border bg-surface text-muted-foreground")} style={{ left: `${placement.x * scaleX}%`, top: `${placement.y * scaleY}%`, width: `${placement.width * scaleX}%`, height: `${placement.height * scaleY}%` }}><span className="max-w-full truncate px-1">{placement.patternId}</span></div>)}
    </div>
  );
}
...
function Stat({ label, value }: { label: string; value: string }) { return <div className="rounded border border-border bg-panel p-2 text-center"><div className="text-lg font-semibold text-primary">{value}</div><div className="text-[9px] uppercase tracking-wide text-muted-foreground">{label}</div></div>; }
